...
def save_schedule(data: dict, date_str: str, base_dir: str = "./data/inputs"):
    if "body" not in data:
        return False
...
def main():
    parser = argparse.ArgumentParser(description='Collect NBA schedule data')
    ...
    for current_date in tqdm(date_range, desc="Collecting schedule"):
        date_str = current_date.strftime('%Y%m%d')

        output_path = get_partitioned_path(base_dir=f"{args.output_dir}/schedule", date=date_str)
        if output_path.exists():
            dates_skipped += 1
            continue

        schedule_response = client.get_daily_schedule(date_str)
        if schedule_response.get('statusCode') != 200:
            continue
        ...
        saved = save_schedule(schedule_response, date_str, args.output_dir)
        if saved:
            dates_processed += 1
