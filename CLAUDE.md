...
- backtest_season.ipynb: Season-long walk-forward validation
...
Active development: Multi-day backtesting, injury filtering, contextual features
...
from src.evaluation.metrics.accuracy import MAPEMetric, RMSEMetric

validator = Validator(
    model=XGBoostModel(),
    metrics=[MAPEMetric(), RMSEMetric()]
)

results = validator.walk_forward_validate(data_splits)
```
